...
class Modularity(commands.Cog):
    ...
    async def whitelistUsers(self, context, *mentions):
        ...
        for user in users: 
            try:
                self.table.insert().values(name=user.name, user_id=user.id).execute()
                added.append(user)
    ...
    async def unwhitelistUser(self, context, *mentions):
        ...
        for user in users: 
            self.table.delete().where(self.table.c.user_id == user.id).execute()
