...
class Modularity(commands.Cog):
    ...
    async def allowed(self, user: discord.User):
        ...
        return str(user) == Modularity.owner or \
                self.table.select().where(
                    self.table.c.discord_user_id == user.id
                    ).execute().rowcount > 1 
    ...
    async def whitelistUsers(self, context, *mentions):
        ...
        for user in users: 
            try:
                self.table.insert().values(name=user.name, discord_user_id=user.id).execute()
                added.append(user)
    ...
    async def unwhitelistUser(self, context, *mentions):
        ...
        for user in users: 
            self.table.delete().where(self.table.c.discord_user_id == user.id).execute()
